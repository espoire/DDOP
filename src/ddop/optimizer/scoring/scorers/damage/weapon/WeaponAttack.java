...
public abstract class WeaponAttack extends DamageSource {
	...
	private static Map<String, Double> generateDamageMods() {
		...
		// DPS value assumed to be the same as fire/air until more data becomes available.
		ret.put("alchemical water attunement", 22.05);
		ret.put("alchemical earth attunement", 22.05);

		// Experimental:
		// 1000 crossbow bolts (assumed 950 hits): 10 fire procs, 13 lightning procs,
		// ~1300-2900 damage each. No apparent damage difference between elements.
		...
		// Summary:
		// 1% on hit, 2205 [type] damage.
		ret.put("alchemical fire attunement", 2205 * 1 * perCent);
		ret.put("alchemical air attunement",  2205 * 1 * perCent);
